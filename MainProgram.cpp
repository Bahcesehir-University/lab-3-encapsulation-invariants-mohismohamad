...
public:
    ...
    explicit Temperature(double celsius) {
        ...
        if (celsius < -273.15){
        	throw invalid_argument("Temprature can not be below absolute zero");
		}
		 celsius_ = celsius;   
    }
...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if (owner.empty()){
        	throw invalid_argument("Owner name can not be empty");
		}
		if(initialBalance < 0){
			throw invalid_argument("Initial Balance can not be negative.");
		}
		owner_ = owner;
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if (pwd.length() < 8){
            throw invalid_argument("Password must be at least 8.");
        }
        if (!hasDigit(pwd)){
            throw invalid_argument("Password must has digit.");
        }
    }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if (oldPassword != password_){
            throw invalid_argument("Password and old password matches.");
        }
